...
func ParseIntegerListToStringSlice(flagName, flagValue string) ([]string, error) {
	...
	for _, part := range commaParts {
		...
		if !strings.Contains(value, "-") {
			...
			if err != nil {
				return values, errors.New(spec.ParameterIllegal.Sprintf(flagName, flagValue, err))
			}
		...
		if len(ranges) != 2 {
			return values, errors.New(spec.ParameterIllegal.Sprintf(flagName, flagValue,
